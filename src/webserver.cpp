...
void WebInterface::begin() {
    ...
    // Registriere die Routen aber OHNE die Update-API-Endpunkte
    ...
    server.on("/firmware", HTTP_GET, [this]() {
        ...
        String html = F("<!DOCTYPE html><html><head>"
            ...
            ".error{color:#f87171;margin:20px 0;padding:10px;background:#374151;border-radius:4px}"
            ".button-link{display:inline-block;margin-top:10px;margin-right:10px;background:#4f46e5;color:white;padding:8px 16px;border-radius:4px;text-decoration:none}"
            ".button-link:hover{background:#4338ca}"
            ".button-link.secondary{background:#6b7280;color:white}"
            ".button-link.secondary:hover{background:#4b5563}"
            ".code{font-family:monospace;padding:10px;background:#374151;border-radius:4px;overflow-x:auto;margin:10px 0;font-size:0.9em;color:#e5e7eb}"
            "</style>"
            "</head><body>"
            "<div class='container'>"
            "<h1>Hollowclock Update</h1>"
            
            "<div class='info'>ℹ️ Current Version: " FW_VERSION "</div>"
            
            "<h2>1. Update Firmware</h2>"
            "<div class='warning'>⚠️ Warning: This updates only the firmware. The filesystem remains unchanged.</div>"
            "<form method='POST' action='/update' enctype='multipart/form-data'>"
            "<input type='file' name='update' accept='.bin'>"
            "<input type='submit' value='Update Firmware'>"
            "</form>"
            
            "<h2>2. Update Filesystem</h2>"
            "<div class='warning'>⚠️ Warning: This updates only the filesystem (web interface). The firmware remains unchanged.</div>"
            "<div class='info'>ℹ️ Note: The filesystem file is usually named 'littlefs.bin'</div>"
            ...
            "<a href='/' class='button-link secondary'>← Back to Home</a>"
            "<a href='https://github.com/sogidaned/Hollowclock/releases' target='_blank' class='button-link'>GitHub Releases</a>"
            "</div>"
            "</div></body></html>");
    ...
    Serial.println("HTTP-Update-Server initialisiert auf /update");
